...
public class ReplaceClassVisitor extends ASTVisitor {
   ...
   public boolean visit(TypeDeclaration node) {
      ...
      checkModifier(node, oldName, isPublicFinal);
      if(isPublicFinal == true) 
      {
    	  if(MsgUtil.openQuestion("Do you want to rename this public (or final) class " + oldName)) 
    	  {
    		  replaceType(oldName);
   ...
   private void checkModifier(TypeDeclaration node, SimpleName oldName, boolean isPublicFinal) {
      ...
      for (Object m : modifiers) {
         if (m instanceof Modifier) {
            ...
            if(modStr.compareTo("public") != 0 || modStr.compareTo("final") != 0) 
            {
            	isPublicFinal = false;
            }
         }
